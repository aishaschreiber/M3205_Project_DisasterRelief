...
def RunningBenders(instance: str, verbose: bool = True):
        ...
        for s in S:
            ...
            if CSP.status != GRB.INFEASIBLE or ReducedCSP.status != GRB.INFEASIBLE:   
                ...
                for l in L:
                    ...
                    DFDSP.setObjective(quicksum(Phi[j] * I[j,l].x for j in PF) + quicksum(Theta[i,k] * R_kl[k,l] * D_bar[(s,i,l)] * Y[i,s].x for i in TF for k in K))
                    ...
                    if DFDSP.status == GRB.UNBOUNDED:
                        ...
                        LIPMP.addConstr(quicksum(Phi[j].UnbdRay * I[j,l] for j in PF) + 
...
if __name__ == "__main__":
    RunningBenders("CLFDIP Model Data Generation Tool\Instances (Sets and Parameters)\Pr01_S2.txt", verbose = True)